          ...
          {({ field: { value, onChange } }) => (
            <div className='border rounded-xl overflow-hidden mt-4'>
              <div className='h-44 flex justify-center items-center bg-zinc-50'>
                {!isLoading ? (
                  value ? <img className='h-full object-cover w-full' src={value} /> : <MountainSnowIcon className='text-zinc-400 size-6'/>
                ) : (
              ...
              <label className='bg-white font-semibold text-red-500 border-t h-14 justify-center cursor-pointer hover:bg-zinc-100 duration-300 transition-colors flex gap-2 items-center'>
                ...
                <input onChange={(e) => onChange(e.target.files && URL.createObjectURL(e.target.files[0]))} type='file' className='sr-only'/>
              </label>
            </div>
          )}
        </Field.Main>
